...
// Get DOM elements
const outputCanvasElement = document.getElementById("canvas");
...
let minConfidence = 0.4, maxConfidence = 0.7;
...
const txtMinGbConf = document.getElementsByClassName("slider-min-cf-value")[0];
...
const txtMaxGbConf = document.getElementsByClassName("slider-max-cf-value")[0];
...
function drawSegmentationResult(result) {
    ...
    const width = outputCanvasElement.width, height = outputCanvasElement.height;
...
async function predictWebcam() {
    if (isFirstFrame) {
        ...
        bgImageData = calculateBackgroundImageData(bgImage, outputCanvasElement.width, outputCanvasElement.height);
    }
    ...
    imageSegmenter.segmentForVideo(outputCanvasElement, nowInMs, drawSegmentationResult);
}
